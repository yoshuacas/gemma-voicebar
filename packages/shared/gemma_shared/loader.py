...
def load():
    ...
    print(f"[load] ready in {elapsed:.1f}s", flush=True)
    load.elapsed = elapsed  # type: ignore[attr-defined]
...
def generate(processor, model, messages, *, max_new_tokens=512, enable_thinking=False):
    ...
    print(f"[gen] {new_tokens.shape[0]} tok in {elapsed:.1f}s ({tps:.1f} tok/s)", flush=True)
    return text
