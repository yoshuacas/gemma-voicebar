...
class VoiceBarApp(rumps.App):
    ...
    def _load_engines(self) -> None:
        ...
        try:
            asr.warmup()
            tts.warmup(self.config.get("voice", "af_heart"))
    ...
    def _build_menu(self) -> None:
        ...
        self.recording_method = rumps.MenuItem(
            "Recording method",
            callback=None,
        )
        self.recording_method["Push-to-talk (Right Option)"] = rumps.MenuItem(
            "Push-to-talk (Right Option)", callback=lambda _: None
        )
        self.recording_method["Push-to-talk (Right Option)"].state = 1

        self.voice_menu = rumps.MenuItem("Voice")
        for v in VOICES:
            mi = rumps.MenuItem(v, callback=self._on_voice_pick)
            mi.state = 1 if v == self.config.get("voice", "af_heart") else 0
    ...
    def _on_ptt_up(self) -> None:
        if not self.recorder.is_recording and not self.recorder._frames:
            return
        ...
        try:
            ...
            if streamer:
                streamer.stop()
            rumps.notification("VoiceBar", "Recording error", str(e))
            self._set_title(ICON_WARN)
            return
    ...
    def _asr_and_inject(
        ...
        try:
            ...
            rumps.notification("VoiceBar", "ASR failed", str(e))
            self._set_title(ICON_WARN)
            threading.Timer(0.8, self._set_title, args=(ICON_IDLE,)).start()
            return
        ...
        if ok:
            ...
            rumps.notification(
                "VoiceBar",
                "Paste blocked",
                "Likely a secure field. Text left on clipboard.",
            )
            self._set_title(ICON_WARN)
            threading.Timer(0.8, self._set_title, args=(ICON_IDLE,)).start()
        ...
        try:
            with self._engine_lock:
                samples, sr = tts.synthesize(
                    text, voice=self.config.get("voice", "af_heart")
                )
        except Exception as e:  # noqa: BLE001
            rumps.notification("VoiceBar", "TTS failed", str(e))
            self._set_title(ICON_WARN)
            threading.Timer(0.8, self._set_title, args=(ICON_IDLE,)).start()
            return
